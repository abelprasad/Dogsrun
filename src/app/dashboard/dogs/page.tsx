    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="bg-[#fffbeb] border-b border-gray-200 py-10 px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Link href="/dashboard/dogs/new" className="inline-block bg-[#f59e0b] text-[#451a03] font-semibold rounded-lg px-5 py-2.5 hover:bg-[#d97706] transition-colors text-sm">
            + Add a dog
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto py-8 px-8">
        {dogs && dogs.length > 0 ? (
          <div className="space-y-3">
            {dogs.map((dog) => (
              <div key={dog.id} className="bg-white border border-gray-100 rounded-xl flex items-center gap-5 px-5 py-4 shadow-sm hover:border-gray-200 transition-colors">
          ...
          <div className="py-24 bg-[#fffbeb] rounded-xl border border-dashed border-gray-200 text-center">
            ...
            <Link href="/dashboard/dogs/new" className="text-[#f59e0b] font-bold hover:underline text-sm">Add your first dog →</Link>
          </div>
        )}

        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-100">
            <p className="text-xs text-[#9ca3af] font-semibold">Page {page} of {totalPages} · {count} dogs</p>
            <div className="flex gap-2">
              {page > 1 && (
                <Link href={`/dashboard/dogs?page=${page - 1}`} className="px-4 py-2 text-xs font-semibold border border-gray-200 rounded-lg text-[#374151] hover:bg-gray-50 transition-colors">← Previous</Link>
              )}
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <Link key={p} href={`/dashboard/dogs?page=${p}`} className={`px-4 py-2 text-xs font-semibold rounded-lg transition-colors ${p === page ? 'bg-[#f59e0b] text-[#451a03]' : 'border border-gray-200 text-[#374151] hover:bg-gray-50'}`}>
                  {p}
                </Link>
              ))}
              {page < totalPages && (
                <Link href={`/dashboard/dogs?page=${page + 1}`} className="px-4 py-2 text-xs font-semibold border border-gray-200 rounded-lg text-[#374151] hover:bg-gray-50 transition-colors">Next →</Link>
              )}
            </div>
          </div>
        )}
