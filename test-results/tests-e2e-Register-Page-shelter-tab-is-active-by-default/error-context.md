...
- Name: tests\e2e.spec.ts >> Register Page >> shelter tab is active by default
- Location: tests\e2e.spec.ts:108:7
...
Locator:  locator('button:text("Shelter")').first()
...
Call log:
  ...
  - waiting for locator('button:text("Shelter")').first()
    9 × locator resolved to <button type="button" class="flex-1 py-3 text-sm font-black uppercase tracking-[0.16em] transition-colors bg-[#f4b942] text-[#13241d]">shelter</button>
